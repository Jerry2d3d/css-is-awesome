...
8. **On phones, interactive surfaces take the FULL WIDTH.** Below the mobile breakpoint, a control spans its column: triggers stretch to 100% (label left, affordance right via `justify-content: space-between`), menus/drawers/sheets span the viewport (minus small gutters — for a top-layer popover set `inset-inline` at `&[popover]` specificity, since the dropdown mixin's `inset: unset` reset otherwise wins). No floating mid-width popups on small screens — `cia.sheet`, `cia.drawer($side: top|bottom)` and `cia.dock` already obey this.
