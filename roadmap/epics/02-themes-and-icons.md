...
> **STATUS — audited 2026-07-16 (v0.8.2). Largely shipped.** `public/themes/` holds 21 theme folders including the suffixed light/dark naming migration (Feature 2.14) and the universal light/dark pairing (Feature 2.13: cupertino / glass / graphite / press / prism / sketchbook all paired, terminal single-mode), plus the `boilerplate` theme (2.12). The vendored Lucide **core** pack shipped (49 glyphs in `public/icons/core/`, Feature 2.10). Live: contrast audit (`scripts/theme-a11y.js`, 2.6), icon-pack contract check (`scripts/icon-validator.js` + `scripts/icon-contract.json`, 2.11), the browser theme editor at `/themes` (2.9), the pairing docs page `/docs/themes/pairing` (2.13.7), and the theme-submission issue template (2.5.1). **Not done / partial:** alternate icon packs Phosphor & Heroicons (2.11 ships `core` only), per-theme size/perf budget CI (2.7), `CONTRIBUTING-ICONS.md` (2.16.3), and the entire add-ons distribution layer (2.16, all P2/post-1.0). Per-criteria `- [ ]` boxes below are **stale**.

> **Update 2026-08-30 — Feature 2.6 is now real; Feature 2.13/2.14 artifacts reconciled.** **US-2.6.1 / US-2.6.2 / US-2.6.3 were reported green on a validator that was not reading the themes.** Its block regex required quotes, so a valid unquoted `[data-theme=dark]` matched nothing, the file fell through to a legacy `:root`-only path, and the audit exited 0 having graded nothing — a theme with near-black text on near-black paper passed. Fixed, plus a guard that **refuses to grade** any file that mentions `data-theme` but parses no block. The audit also grew from 17 to **22 pairs per theme** (the five `--code-*` foregrounds against `--code-bg` had never been checked), which surfaced **33 genuine failures across 9 themes**, worst 3.02:1 — all now fixed. `prism`'s `--code-muted` needed `light-dark()`: no single grey clears AA against both `#fafafa` and `#0a0a0a`.
>
> Theme inventory is now **24, each with exactly one SCSS source**. The 13 `-light`/`-dark` files from Feature 2.13 were hand-authored CSS that nothing could rebuild and had drifted 31 declarations; they were **ported, not derived** (9 of 13 were deliberately retuned, e.g. `glass-light`'s `blur(30px)` vs the parent's `blur(20px)`). `sketchbook-light`, `boilerplate-light` and `boilerplate-dark` existed *only* as blocks inside the bundled `public/theme.css` — invisible to the contract gate — and were promoted to sources. A drop-in `theme.css` no longer requires a matching `<html data-theme>`: `theme()` emits `:root, :root[data-theme="<name>"]`. Still open: per-theme size/perf budget CI (2.7), Phosphor/Heroicons packs (2.11), add-ons layer (2.16).
