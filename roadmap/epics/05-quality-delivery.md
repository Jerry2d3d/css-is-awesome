...
> **STATUS — audited 2026-07-16 (v0.8.2). Partial — and two features are SUPERSEDED.** **Shipped:** GitHub Actions CI (`.github/workflows/ci.yml`) gating lint + lint:scss + validate-themes + build:css:all + `next build` + Playwright (Feature 5.12); Playwright suites for smoke, a11y (axe), and visual regression with committed `tests/__screenshots__` baselines (5.3 / 5.2 / 5.4 — via Playwright, not Chromatic); contrast audit as `scripts/theme-a11y.js` (5.5); release automation (`.github/workflows/release.yml`, 5.15); npm publish hygiene via `exports`/`files` (5.14); Pages deploy (`.github/workflows/pages.yml`, 5.13); TypeScript token defs (`scripts/generate-token-types.mjs` → `tokens.d.ts`, 5.16). **SUPERSEDED:** Storybook install + stories (5.17, 5.18) — Storybook was explicitly **killed** (no `.storybook/`; the docs site + MCP server cover both the human and AI audiences). **Not shipped:** the Jest unit-test harness, jest-axe unit assertions, and coverage gates (5.1 / 5.2.1 / 5.11 — there is no Jest; the a11y gate is Playwright-level only), bundle-size monitoring (5.7 — no `scripts/bundle-size.js`), and the Lighthouse baseline (5.6). Per-criteria `- [ ]` boxes below are **stale**.

> **Update 2026-08-30 — Feature 5.5 was passing without running.** `scripts/theme-a11y.js` only matched *quoted* `[data-theme="x"]` selectors; an unquoted `[data-theme=dark]` block matched nothing, the file fell back to a legacy `:root`-only path, and the audit exited 0 reporting "1 theme block(s)" — so **US-5.5.1 and US-5.5.3 were green on themes the script never parsed**. Fixed, with a guard that refuses to grade rather than fall back. Coverage expanded 17 → **22 pairs** (`--code-ink/muted/accent/blue/green` vs `--code-bg`), surfacing 33 real failures across 9 themes, all fixed. All **24** themes now pass for real.
>
> Also new in CI: **`npm run check:theme-drift`** (`scripts/check-theme-drift.mjs`). `validate-themes` reads the committed CSS under `public/`, and theme building was in no CI step, so a fix in `scss/themes/*.scss` that was never rebuilt would validate stale output and report success. The gate rebuilds into a scratch copy, diffs declaration-by-declaration, and restores. `build:css:themes` also regenerates the bundle now and is finally part of `build:css:all`; the one-shot `scripts/bundle-companion-themes.mjs` (which only ever *appended* missing blocks, letting the bundle fall 8 commits behind) is deleted.

> **Update 2026-09-01 — release automation (5.15), publish hygiene (5.14), Pages deploy (5.13), and visual regression (5.4) are all proven in production.** semantic-release published **`css-is-awesome@1.1.0` to npm on 2026-09-01 — the first publish in the project's history** — end to end through the CI → Release workflows (dist-tag `latest`, GitHub Release, annotated tag); jsDelivr and unpkg `@1` URLs verified resolving. The docs site is **live at <https://jerry2d3d.github.io/css-is-awesome/>**, auto-deploying on every push to `main` (repo public as of 2026-09-01). Visual baselines are now committed for **both win32 and linux** (linux set regenerated 2026-08-29/30, d8a3974 + 093cb62, after fixing the snapshot workflow that discarded its own baselines) — the visual suite is green on both platforms. Still not built: Jest unit harness, jest-axe assertions, coverage gates, bundle-size monitoring, Lighthouse baseline.
