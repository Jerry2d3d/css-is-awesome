...
> **STATUS — audited 2026-07-16 (v0.8.2). Strong — largely shipped.** MCP server at `mcp/server.cjs` (bin `css-is-awesome-mcp`, ~29 tools incl. list/get tokens, mixins, components, `resolve_size`, `assemble_prompt`, `search_components`, `validate_theme`) (Feature 6.1); the `cia` CLI shipped as `bin/cia.cjs` (bin `cia`) rather than a separate `@css-is-awesome/cli` package (6.2); JSON token export in `figma-tokens/` (6.3); `llm.txt` at repo root + served (6.6); AI-agent instruction files `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` plus the deep `css-is-awesome.instructions.md` (6.9); `/docs/mcp` + `/docs/composition` pages. **Not shipped:** the reusable prompt-template folder `prompts/` (6.4), hosted custom Claude/ChatGPT/Gemini bots (6.5), and a unified `/docs/ai` integration page (6.8 — `/docs/mcp` covers only the MCP slice). Per-criteria `- [ ]` boxes below are **stale**.
