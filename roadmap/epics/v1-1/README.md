...
| [07](./EPIC-07-mcp-zero-install.md) | **MCP that just works** | `npx css-is-awesome-mcp` with no manual `sdk`+`zod` install — bundle into the server artifact or ship an add-on package, without adding JS deps to the core package. From the 2026-09-07 external review; pull-forward DX. **🟡 Partial 2026-09-11** — the new sibling-repo package is built and verified locally (real npm-registry install, identical data to the dev-tree server); publishing + the user-facing doc/error-message updates are the genuinely remaining step, deliberately deferred (see the epic file's status note). | ~1-2 days | 4 |
