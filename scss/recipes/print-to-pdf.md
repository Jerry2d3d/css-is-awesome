...
- **Animations snapshot invisible.** Entrance fades and scroll reveals often start at `opacity: 0`; a PDF captured mid-animation prints **blank**. `print-base` freezes all animation/transition and forces `opacity: 1` by default — keep it on unless you have a specific reason not to.
