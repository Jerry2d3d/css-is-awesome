...
{"\n"}{"}"}</Example.Code>
      ...
      <p>In <code>@media print</code> it always sets <code>color-scheme: light</code>, so paired <code>light-dark()</code> themes print their light branch for free. It also defines a themeable <strong>print palette</strong> — <code>--print-ink</code>, <code>--print-paper</code>, <code>--print-line</code>, <code>--print-muted</code> (default ink-on-white) — and rebinds the theme&rsquo;s own colour tokens (<code>--ink</code>, <code>--surface-*</code>, <code>--border-*</code>, <code>--code-*</code>) onto it, so every theme — dark-only like Terminal included — prints legible ink-on-paper; override a <code>--print-*</code> token to restyle paper. Three opt-in flags (all default off) go further: <code>$link-urls</code> prints each link&rsquo;s destination via <code>attr(href)</code>; <code>$link-origin</code> prepends an origin to internal links so they resolve to full URLs on paper; <code>$page-numbers</code> puts <code>counter(page)</code> in the <code>@page</code> bottom-center margin box. (<code>$legible</code> is a deprecated no-op — the rebind supersedes it.) Full walkthrough on the <Link href="/docs/print">Print page</Link>.</p>
      <Example>
...
{"\n"}<span className="tok-prop">@include</span> <span className="tok-val">m.print-base($link-urls: true, $link-origin: &apos;https://example.com&apos;, $page-numbers: true)</span>;</Example.Code>
      </Example>
