"use client";
// Client gallery for /docs/recipes. Kept a client component because it renders
// compound client components — Tabs.List/Trigger/Panel and DataTable's function
// `rowKey` — whose statics/functions don't survive being accessed from a Server
// Component. The markdown recipe list is read server-side in page.tsx and
...
import ProjectsTableExample from "./_components/ProjectsTableExample";

// Card title is pre-computed server-side (prettifyRecipeName lives in the
// fs-backed lib module, which can't be imported here), so cards render strings.
...
export default function RecipesGallery({ recipes }: { recipes: RecipeCard[] }) {
  return (
    <>
      <h1>Recipes</h1>
      <p className="lead">
      ...
      {recipes.length > 0 && (
        <section>
          <h2 id="recipe-library">Recipe library</h2>
          <p>
            Full, accessible recipes with raw markup, SCSS wiring, and framework
            notes. Each opens its own page with copy-paste code blocks.
          </p>
          <div className="recipe-card-grid">
            {recipes.map((r) => (
              <Link key={r.slug} href={`/docs/recipes/${r.slug}`} className="recipe-card">
                <span className="recipe-card-title">{r.title}</span>
                <span className="recipe-card-desc">{r.description}</span>
                <span className="recipe-card-meta">
                  {r.category && <span className="recipe-chip">{r.category}</span>}
                  {r.complexity && (
                    <span className="recipe-chip" data-complexity={r.complexity}>
                      {r.complexity}
                    </span>
                  )}
                </span>
