...
{"\n"}      <span className="tok-prop">media</span>=<span className="tok-val">"(prefers-color-scheme: dark)"</span><span className="tok-sel">{">"}</span></Example.Code>
      ...
      <p>
        No <code>data-theme</code> needed on <code>{`<html>`}</code>. Each shipped theme file emits{" "}
        <code>:root, :root[data-theme=&ldquo;sketchbook&rdquo;]</code> — the bare{" "}
        <code>:root</code> is what makes a single file apply on its own, and here the{" "}
        <code>media</code> attribute does the gating for you. Include both files and let the OS
        preference decide.
      </p>
      <p>
        Use the per-theme files for this, not the 24-theme bundle. The bundle has the bare{" "}
        <code>:root</code> stripped precisely so its themes don&rsquo;t collide, which means it needs
        the attribute and can only ever express one theme at a time.
      </p>
      ...
      <ul>
        ...
        <li>...any of the 8 theme families &times; 8 = 64 combinations, or 56 once you drop the self-pairs — and 24 files to draw from if you want to pin specific light/dark variants</li>
      </ul>

      <h2 id="constraints">What you should know</h2>
      <ul>
        <li><strong>The browser loads both files.</strong> Individual themes run 1.9&ndash;2.7 KB gzipped, so a pairing lands around 4&ndash;5 KB. No conditional download — just conditional application.</li>
...
{"\n"}<span className="tok-sel">{"<html"}</span> <span className="tok-prop">class</span>=<span className="tok-val">"cia-force-light"</span><span className="tok-sel">{">"}</span>...<span className="tok-sel">{"</html>"}</span></Example.Code>
      ...
      <ul>
        <li><a href="/themes">Browse the eight theme families</a></li>
